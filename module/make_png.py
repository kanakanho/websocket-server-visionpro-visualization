...
def plot_savefig(df: pd.Series, latest_file: str):
    ...
    linei_width = 4 if df["distance_sum"] <= 0.45 else 1
    ...
    # unixtimeからdatetime（m秒含む 例:1736155283.755259 → 2025 ~~~ に変換
    dt = datetime.datetime.fromtimestamp(df["time"])
    plt.title(f"Time: {dt}")
...
def make_png(isOne: bool = False):
    ...
    if isOne:
        # 真ん中の1つをプロット
        df_cp = df.loc[df.shape[0] // 2]
        plot_fig(df_cp, latest_file)
        return
...
def plot_fig(df: pd.Series, latest_file: str):
    # 関節同士のつながりをプロット
    fig = plt.figure(figsize=(8, 8))
    ax = fig.add_subplot(111, projection="3d")
